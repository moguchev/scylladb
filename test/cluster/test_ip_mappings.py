...
@pytest.mark.asyncio
@skip_mode('release', 'error injections are not supported in release mode')
async def test_bootstrapping_node_restarts_with_new_ip(manager: ManagerClient):
    """
    ...
    await manager.server_stop(new_server.server_id)
...
@pytest.mark.asyncio
async def test_peers_table_read_fixup_does_not_deadlock(manager: ManagerClient):
    """
